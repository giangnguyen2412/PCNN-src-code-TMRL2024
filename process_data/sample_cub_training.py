...
train_path = f'{RunningParams.parent_dir}/datasets/CUB_train'
val_path = f'{RunningParams.parent_dir}/datasets/CUB_val'
test_path = f'{RunningParams.parent_dir}/datasets/CUB_test'
...
for folder_name in os.listdir(f'{RunningParams.parent_dir}/datasets/CUB/test0'):

    # Skip non-directory files
    if not os.path.isdir(os.path.join(f'{RunningParams.parent_dir}/datasets/CUB/test0', folder_name)):
        continue

    # Get the list of files in the current folder, excluding the test files
    files = os.listdir(os.path.join(f'{RunningParams.parent_dir}/datasets/CUB/test0', folder_name))
    tmp_list = []
    for file in files:
...
for folder_name, file_name in train_files:
    src_path = os.path.join(f'{RunningParams.parent_dir}/datasets/CUB/test0', folder_name, file_name)
