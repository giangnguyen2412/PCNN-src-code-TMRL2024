...
train_tmp_path = f'{RunningParams.parent_dir}/Final_RN50_dataset_CUB_LP/train'
val_tmp_path = f'{RunningParams.parent_dir}/Final_RN50_dataset_CUB_LP/val'
...
for folder_name in os.listdir(f'{RunningParams.parent_dir}/datasets/CUB/combined'):

    # Skip non-directory files
    if not os.path.isdir(os.path.join(f'{RunningParams.parent_dir}/datasets/CUB/combined', folder_name)):
        continue
    ...
    files = os.listdir(os.path.join(f'{RunningParams.parent_dir}/datasets/CUB/combined', folder_name))
...
for folder_name, file_name in train_files:
    src_path = os.path.join(f'{RunningParams.parent_dir}/datasets/CUB/combined', folder_name, file_name)
    os.makedirs(os.path.join(train_tmp_path, folder_name), exist_ok=True)
