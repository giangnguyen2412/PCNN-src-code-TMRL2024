...
num_samples_to_add = 8144 - num_original_test_samples  # Calculate how many samples you need to add
...
padded_test_loader = torch.utils.data.DataLoader(
    padded_test_data,
    batch_size=509,  # Keep the batch size consistent with your original setup
    shuffle=False,
    num_workers=8,
    pin_memory=True,
)
...
all_similarity_scores = np.zeros((8144, len(train_data)))  # Use 5994 here for the padded size
num_test_samples = 15
...
def compute_similarity_chunked(transformer_model, query_images, reference_images):
    ...
    for query_img in query_images:
        query_scores = []
        for ref_chunk in torch.split(reference_images, 509):  # Adjust chunk size as needed
...
np.save('car_knn_algorithm_S_scores.npy', all_similarity_scores_np)

print("Saved all_similarity_scores to disk as 'car_knn_algorithm_S_scores.npy'")
