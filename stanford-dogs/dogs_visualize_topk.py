...
if __name__ == '__main__':
    ...
    for ds in ['cub_test']:
            ...
            for sample_idx in range(x.shape[0]):
                ...
                if (is_correct and len(correct_figures) < sample_num) or (not is_correct and len(incorrect_figures) < sample_num):
                    ...
                    for i, pred in enumerate(final_preds):
                        ...
                        class_name = ''.join(data_loader.dataset.classes[pred].split('-')[1:])
                        color = 'black'
                        if i == 0:
                    ...
                    if TOP1_NN is True:
                        save_path = f'{RunningParams.prj_dir}/corrections/dogs/top1/{class_name}_{batch_idx}_{sample_idx}.jpeg'
                    else:
                        save_path = f'{RunningParams.prj_dir}/corrections/dogs/topk/{class_name}_{batch_idx}_{sample_idx}.jpeg'
        ...
        if TOP1_NN is True:
            correctness_file_path = f'{RunningParams.prj_dir}/corrections/dogs/top1/correctness_info.json'
        else:
            correctness_file_path = f'{RunningParams.prj_dir}/corrections/dogs/topk/correctness_info.json'

        with open(correctness_file_path, 'w') as json_file:
            json.dump(correctness_dict, json_file)

        if TOP1_NN is True:
            metadata_path = f'{RunningParams.prj_dir}/corrections/dogs/top1/metadata.json'
        else:
            metadata_path = f'{RunningParams.prj_dir}/corrections/dogs/topk/metadata.json'
