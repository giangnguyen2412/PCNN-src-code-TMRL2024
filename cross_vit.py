...
class CrossViT(nn.Module):
    def __init__(
        ...
        *,
        ...
        # self.mlp_head = nn.Sequential(nn.LayerNorm(lg_dim + sm_dim + 1),
        #                               nn.Linear(lg_dim + sm_dim + 1, (lg_dim + sm_dim)//2),
        #                               nn.LayerNorm((lg_dim + sm_dim)//2),
        #                               nn.Linear((lg_dim + sm_dim)//2, (lg_dim + sm_dim)//4),
        #                               nn.LayerNorm((lg_dim + sm_dim) // 4),
        #                               nn.Linear((lg_dim + sm_dim)//4, (lg_dim + sm_dim)//num_classes),
        #                               )

        self.fc0 = nn.Linear(lg_dim + sm_dim + 1, (lg_dim + sm_dim)//2)
        self.fc0_bn = nn.BatchNorm1d((lg_dim + sm_dim)//2, eps=1e-2)
        self.fc1 = nn.Linear((lg_dim + sm_dim)//2, (lg_dim + sm_dim)//4)
        self.fc1_bn = nn.BatchNorm1d((lg_dim + sm_dim)//4, eps=1e-2)
        self.fc2 = nn.Linear((lg_dim + sm_dim)//4, num_classes)
    ...
    def forward(self, img1, img2):
        ...
        # sep_token = nn.Parameter(torch.randn(sm_cls.shape[0], 1)).cuda()
        sep_token = torch.ones([sm_cls.shape[0], 1]).cuda()
        feat = torch.concat([sm_cls, sep_token, lg_cls], dim=1)

        output = self.dropout(self.fc0_bn(torch.nn.functional.relu(self.fc0(feat))))
        output = self.dropout(self.fc1_bn(torch.nn.functional.relu(self.fc1(output))))
        logits = self.dropout(self.fc2_bn(torch.nn.functional.relu(self.fc2(output))))
