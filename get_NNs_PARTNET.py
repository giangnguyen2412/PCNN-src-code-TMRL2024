...
import torch
running_corrects = 0
filename = '/home/giang/Downloads/advising_network/PART/results.npy'
cnt_dict = {}
...
for k, v in kbc.items():
    ...
    model1_p = torch.nn.functional.softmax(preds, dim=1)
    score, index = torch.topk(model1_p, 1, dim=1)

    running_corrects += torch.sum(index.squeeze() == torch.tensor(gts).cuda())

acc = running_corrects*100/5794
print(acc)
